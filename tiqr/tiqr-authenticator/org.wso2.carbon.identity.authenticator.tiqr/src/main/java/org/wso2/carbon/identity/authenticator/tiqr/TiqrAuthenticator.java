    ...
    protected void processAuthenticationResponse(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            if (request.getParameter(TiqrConstants.TIQR_ACTION).equals(TiqrConstants.TIQR_ACTION_AUTHENTICATION)) {
                ...
                while (retry < retryCount) {
                    String checkStatusResponse = sendRESTCall(urlToCheckAuthentication, "",
                            "action=getAuthenticatedUser&sessId=" + request.getParameter(TiqrConstants.ENROLL_SESSIONID),
                    ...
                    if (checkStatusResponse.contains("<p>Hello ")) {
                        userId = checkStatusResponse.substring(checkStatusResponse.indexOf("<p>Hello "),
                                checkStatusResponse.indexOf(".</p>")).replace("<p>Hello ", "").trim();
                ...
                while (retry < retryCount) {
                    ...
                    status = Integer.parseInt(checkStatusResponse
                            .substring(checkStatusResponse.indexOf("Enrolment status: "),
            ...
            if (status == Integer.parseInt(TiqrConstants.ENROLLMENT_SUCCESS_STATUS)) {
                ...
                log.info(request.getParameter(TiqrConstants.TIQR_ACTION) + " success");
            } else {
