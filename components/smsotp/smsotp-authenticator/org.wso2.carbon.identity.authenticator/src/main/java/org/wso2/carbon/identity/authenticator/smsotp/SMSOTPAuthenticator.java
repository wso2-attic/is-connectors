...
public class SMSOTPAuthenticator extends AbstractApplicationAuthenticator
	...
	public boolean canHandle(HttpServletRequest request) {
		...
		return StringUtils.isNotEmpty(request
				.getParameter(SMSOTPConstants.CODE));
	}
	...
	protected void initiateAuthenticationRequest(HttpServletRequest request,
		...
		String secret = OneTimePassword
				.getRandomNumber(SMSOTPConstants.SECRET_KEY_LENGTH);
		otpToken = token.generateToken(secret,
		...
		String httpMethod = authenticatorProperties
				.get(SMSOTPConstants.HTTP_METHOD);
		String headerString = authenticatorProperties
				.get(SMSOTPConstants.HEADERS);
		String payload = authenticatorProperties.get(SMSOTPConstants.PAYLOAD);
		String httpResponse = authenticatorProperties
				.get(SMSOTPConstants.HTTP_RESPONSE);

		String loginPage = ConfigurationFacade
				.getInstance()
				.getAuthenticationEndpointURL()
				.replace("authenticationendpoint/login.do",
						SMSOTPConstants.LOGIN_PAGE);
		...
		if (username != null) {
			...
			username = MultitenantUtils.getTenantAwareUsername(String
					.valueOf(username));
			if (userRealm != null) {
				try {
					mobile = userRealm
							...
							.getUserClaimValue(username,
									SMSOTPConstants.MOBILE_CLAIM, null)
							.toString();
				} catch (UserStoreException e) {
					throw new AuthenticationFailedException(
							"Cannot find the user claim for mobile "
									+ e.getMessage(), e);
				}
				try {
					savedOTPString = userRealm
							.getUserStoreManager()
							.getUserClaimValue(username,
									SMSOTPConstants.SAVED_OTP_LIST, null)
							.toString();
				} catch (UserStoreException e) {
					throw new AuthenticationFailedException(
							"Cannot find the user claim for OTP list "
									+ e.getMessage(), e);
				}
			}
		}
		...
		if (StringUtils.isEmpty(mobile)) {
			...
			try {
				...
				throw new AuthenticationFailedException(
						"Error while sending the HTTP request", e);
			}
		}
	}
	...
	protected void processAuthenticationResponse(HttpServletRequest request,
		...
		if (userToken.equals(contextToken)) {
			...
			savedOTPString = savedOTPString.replaceAll(userToken, "")
					.replaceAll(",,", ",");
			...
			if (username != null) {
				...
				if (userRealm != null) {
					try {
						userRealm.getUserStoreManager().setUserClaimValue(
								username, SMSOTPConstants.SAVED_OTP_LIST,
								savedOTPString, null);
					} catch (UserStoreException e) {
						log.error(
								"Unable to set the user claim for OTP List for user "
										+ username, e);
					}
				}
			}
		} else if (savedOTPString == null) {
			log.error("The claim " + SMSOTPConstants.SAVED_OTP_LIST
					+ " does not contain any values");
		} else {
			log.error("Verification Error due to Code Mismatch");
			throw new AuthenticationFailedException(
					"Verification Error due to Code Mismatch");
		}
	}
	...
	private String getUsername(AuthenticationContext context) {
		...
		for (Integer stepMap : context.getSequenceConfig().getStepMap()
				.keySet())
			if (context.getSequenceConfig().getStepMap().get(stepMap)
					.getAuthenticatedUser() != null
					&& context.getSequenceConfig().getStepMap().get(stepMap)
							.getAuthenticatedAutenticator()
							.getApplicationAuthenticator() instanceof LocalApplicationAuthenticator) {
				username = String.valueOf(context.getSequenceConfig()
						.getStepMap().get(stepMap).getAuthenticatedUser());
				break;
			}
		return username;
	}
	...
	public boolean sendRESTCall(String smsUrl, String httpMethod,
			String headerString, String payload, String httpResponse)
		...
		try {
			smsUrl = smsUrl.replaceAll("\\$ctx.num", mobile).replaceAll(
					"\\$ctx.msg", smsMessage.replaceAll("\\s", "+") + otpToken);
			...
			if (!headerString.trim().isEmpty()) {
				headerString = headerString.replaceAll("\\$ctx.num", mobile)
			...
			if (httpMethod.trim().toLowerCase()
					.equals(SMSOTPConstants.GET_METHOD.toLowerCase())) {
				connection.setRequestMethod(SMSOTPConstants.GET_METHOD);
			} else if (httpMethod.trim().toLowerCase()
					.equals(SMSOTPConstants.POST_METHOD.toLowerCase())) {
				connection.setRequestMethod(SMSOTPConstants.POST_METHOD);
				if (!payload.trim().isEmpty()) {
					payload = payload.replaceAll("\\$ctx.num", mobile)
							.replaceAll("\\$ctx.msg", smsMessage + otpToken);
				}
				...
				try {
					...
					throw new AuthenticationFailedException(
							"Error while posting payload message", e);
				} finally {
					writer.close();
				}
			}
			...
			throw new AuthenticationFailedException(
					"Error while setting the HTTP method", e);
		} catch (IOException e) {
			throw new AuthenticationFailedException(
					"Error while setting the HTTP response", e);
		} finally {
