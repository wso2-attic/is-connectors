...
<?php   } else { ?>
    ...
    <img alt="QR" src="<?php echo sspmod_authTiqr_Auth_Tiqr::generateAuthQR($_REQUEST["AuthState"]); ?>" id="QR"/> <br/>
<?php   }
...
?>
    ...
    if (isset($_POST['action']) and $_REQUEST['action'] == "getAuthenticatedUser") {
        ...
        ?>
        <input type="hidden" name="authenticatedUser" value="<?php echo !empty($user) ? $user : ""; ?>" id="authenticatedUser"/>
        <?php
    }
    ?>
